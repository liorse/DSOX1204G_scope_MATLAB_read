...
% VISA_Instrument This class defines instrument remote-control connection over VISA.NET. Version 1.9.0
% MATLAB Instrument Control Toolbox is not required

% Version 1.9.0
% - Added support for RSNRP sessions
...
% - first version created
    ...
    properties
        ...
        NrpSession % If true, the session is a Nrp-Zxx session
    end
    ...
    methods
        function obj = VISA_Instrument(resourceString)
            ...
            hwIntfType = char(obj.Session.HardwareInterfaceType);
            if (strcmp(hwIntfType, 'Serial') == true)
                obj.VXIcapable = false;
            end
            
            % NRP sessions detection
            if (strcmp(hwIntfType, '33024') == true)
                obj.NrpSession = true;
            end
            ...
            if (obj.NrpSession == true)
                obj.VXIcapable = false;
                obj.ReadBufferSize = 16192;
            else
        ...
        function data = ReadBinaryDataBlock(obj)
            ...
            if (obj.NrpSession == false)
                ...
                % NRPsession - Parse data header in one read, NRP does not support partial reading
                allData = obj.Read(obj.ReadBufferSize);
                hash = char(allData(1));
                if char(hash) ~= '#'
                    throw(MException('VISA_Instrument:Parse_BinaryBlockHeader',...
                    'Expecting # (hash) character at the beginning of the binary data block, received "%s"', char(allData)));
                end
                
                lenOflen = str2double(char(allData(2)));
                blockLen = str2double(char(allData(3 : 3 + lenOflen - 1)));
                data = allData(3 + lenOflen : 3 + lenOflen + blockLen - 1);
            end
